...
* [Using flatcc](#using-flatcc)
  *   [Using flatcc in your CMakeLists.txt](#using-flatcc-in-your-cmakeliststxt)
...
## Using flatcc in your CMakeLists.txt

If your project uses the CMake build system, you can include the cmake module
...
This function sets the following targets and variables:

- `flatcc_generated::<name>` target: linking to this target will automatically
call flatcc for re-generating the sources when the input definition files
have changed

- `<name>_GENERATED_SOURCES` variable: list of all sources that are generated by flatcc.
These can be used by adding they to `add_library`/`add_executable`.

- `flatcc_generated_<name>` target: this target can be used as target of your
make program to re-generate the flatcc headers when an input source has changed.
...
Example:
    ...
    find_package(flatcc REQUIRED COMPONENTS cli runtime)
...
Cross building with flatbuffers on cmake is possible by providing the install location of the build architecture flatcc package via environment variable flatccCli_ROOT or via cmake variable flatccCli_ROOT. The flatcc_generate_sources function will then use the *build* architecture flatcc cli executable to generate the header files and link your target to the *host* architecture libflatccrt library. See section [Cross-compilation](#cross-compilation) for an example.
...
If you're using CMake as a build system, a utility function exists to easily integrate
flatcc. See [the documentation of flatcc_generate_sources](#use-in-cmake-build)
for a short overview + example.
...
The
[`flatcc/flatcc_identifier.h`](include/flatcc/flatcc_identifier.h)
...
*Building the tests require at least CMake 3.3. Passing `-DFLATCC_TEST=OFF`
to cmake disables building them.
